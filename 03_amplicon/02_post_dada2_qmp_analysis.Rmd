---
title: "QMP anlaysis of Db-MM (post_dada2)"
...
# Diet based minimal microbiome (Db-MM-10)study 16S analysis
...
ps <- readRDS("output/rds/pseq01.rds")
#ps <- pseq01
...
write_phyloseq(ps, "OTU", "output/tables" )
#abundances(ps)
```
...
ggsave("output/figs/02_raw_Seq_depth.pdf", 
       height = 6, 
       width = 8)
```
...
saveRDS(ps1, "output/rds/02_CP_corrected_ps1.rds") 

write_phyloseq(ps1, "OTU", path = "output/tables/")
...
ps1 <- readRDS("output/rds/02_CP_corrected_ps1.rds")
#sample_data(ps1)
df.met <- meta(ps1)
#otu_table(ps1)
...
source("codes/qmp_edit.r")

# read the qPCR counts table
counts_tables <- read.csv("data/copies_rrna_counts_qpcr.csv", 
                          row.names = 1)
...
saveRDS(ps1.qmp, "output/rds/02_ps1.qmp.rds")
```

## Visualise variation over time

```{r}
ps1.qmp <- readRDS("output/rds/02_ps1.qmp.rds")
ps1.qmp
med <- meta(ps1.qmp)
unique(med$SampleID)
...
ggsave("output/figs/02_Fig_QMP_Composition_line_plot.pdf", height = 3, width = 7)
write.csv(ps.df.qmp, "output/tables/QMP_plot_data.csv")
```
